...
router.post("/careerGuidance", async (req, res) => {
  try {
...
`;
    ...
    try {
      ...
      return res.status(500).json({
        message: "AI returned response that was not a valid json",
...
router.post("/resumeAnalyzer", async (req, res) => {
  try {
    ...
    if (!pdfBase64) {
      return res.status(400).json("PDF data is required");
    }

    const resumeAnalyzerPrompt = `
